...
export default function MoviesPage() {
  ...
  useEffect(() => {
    fetch("/api/media")
      ...
      .then((d) => {
        const all = d.items || [];
        setItems(all.filter((i: MediaItem) => i.type === "Movie"));
      })
      .catch(() => {})
  ...
  const sortedItems = useMemo(() => {
    ...
    switch (sortBy) {
      case "title":
        ...
        break;
      case "rating":
        sorted.sort((a, b) => (b.communityRating || 0) - (a.communityRating || 0));
        break;
      case "year":
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-text-secondary mt-1">
            {items.length} {t("admin.movieCount").toLowerCase()}
          </p>
        </div>
        <button
          onClick={() => setShowFilters(!showFilters)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-200 hover:bg-surface-hover"
          style={{
            background: showFilters ? "var(--gold-glow)" : "var(--surface)",
            border: `1px solid ${showFilters ? "var(--gold)" : "transparent"}`,
          }}
        >
      ...
      {showFilters && (
        <motion.div
          ...
          className="mb-8 p-4 rounded-xl glass-card flex flex-wrap gap-4"
        >
          <div>
            ...
            <div className="flex gap-2">
              {sortOptions.map((opt) => (
                <button
                  ...
                  style={{
                    background: sortBy === opt.value ? "var(--gold)" : "var(--surface)",
                    color: sortBy === opt.value ? "var(--deep-black)" : "var(--text-secondary)",
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {sortedItems.map((item, i) => (
            <MediaCard key={item.id} item={toNormalized(item)} index={i} />
          ))}
        </div>
      )}
